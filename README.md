...
로그아웃은 **파일** 메뉴에 있다. 제목 표시줄 X 버튼도 같은 동작이다.

배포된 곳: <https://oneline-retro.dq7wl11l9whxd.amplifyapp.com>
...
- **가입 인증 메일이 스팸함으로 자주 간다.** Cognito 기본 발신자
  (`no-reply@verificationemail.com`)는 도메인 평판이 없다. 하루 50통 한도도
  있다. 제대로 고치려면 도메인을 인증해 SES 로 보내야 한다. 인프라는 열어
  두었으니 발신 주소만 넘기면 된다.
